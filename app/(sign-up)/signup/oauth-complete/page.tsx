...
export default function OAuthSignupCompletePage() {
  const oauth = useOAuthComplete({
    successRedirectPath: "/signup/resume-upload",
    errorRedirectPath: "/signup",
    shouldPatchProfile: true,
    logLabel: "signup/oauth-complete",
  });
          ...
          {oauth.isSyncing ? (
            <>
              ...
              <button
                ...
                disabled={oauth.isSyncing}
                className="w-full rounded-lg border border-slate-300 bg-white py-3 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C27803] focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-70"
              >
                Cancel and sign out
              </button>
            </>
            ...
            <>
              ...
              <p className="text-center text-xs text-slate-700">
                Don&apos;t worry — your account is safe. You can log in anytime
                to complete setup.
              </p>
            </>
          ) : null}
